...
public class EventConsumer implements CommunityConstant {
    ...
    public void receiveMessage(ConsumerRecord record){
        ...
        // 消费者得到一个event对象，需要将消息进行转换，并存储到message表中，系统消息的 from_id为1，conversationId为topic
        // 注意此时存入表中的content是 为了能正确显示系统通知 所需要的数据组成的json字符串{entityType,entityId,postId,userId}(map转换成的json字符串)
